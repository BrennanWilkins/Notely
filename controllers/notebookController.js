...
exports.getNotebooks = (req, res, next) => {
  Notebook.find({ 'userId' : req.params.userId }).exec((err, notebooks) => {
...
exports.createNotebook = (req, res, next) => {
  const notebook = new Notebook({ title: req.body.title, userId: req.body.userId, default: false });
...
exports.updateNotebook = (req, res, next) => {
  ...
  const notebook = new Notebook({ title: req.body.title, default: req.body.default, userId: req.body.userId, _id: req.params.id });
...
exports.setDefaultNotebook = (req, res, next) => {
  ...
  Notebook.findByIdAndUpdate(req.params.id, notebook, {}, (err, result) => {
    if (err) { return res.status(500).json({ msg: 'Failed to update notebook.' }); }
    Notebook.updateMany({ $and: [{ userId: req.body.userId }, { _id: { $ne: req.params.id } }]}, { $set: { default: false }}, error => {
      if (err) { return res.status(500).json({ msg: 'Failed to update notebook.' }); }
      return res.status(200).json({ msg: 'Success.' });
    });
  });
